...
int F413ZH_SD_BlockDevice::program(const void *buffer, bd_addr_t addr, bd_size_t size) {
  ...
  if (BSP_SD_WriteBlocks(data_ptr, addr, nBlocks, SD_DATATIMEOUT) == MSD_OK) {
    //SD write OK
      return BD_ERROR_OK;
  } else {
    //Write failed
...
int F413ZH_SD_BlockDevice::read(void *buffer, bd_addr_t addr, bd_size_t size) {
  int nBlocks = size / get_program_size();
  uint32_t *data_ptr = (uint32_t *) buffer;
  if (BSP_SD_ReadBlocks(data_ptr, addr, nBlocks, SD_DATATIMEOUT) == MSD_OK) {
    //SD read OK
    return BD_ERROR_OK;
  } else {
    //read failed
...
bd_size_t F413ZH_SD_BlockDevice::get_program_size() {
  return 512;
}

bd_size_t F413ZH_SD_BlockDevice::get_read_size() {
  return 512;
}
